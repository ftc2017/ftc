...
class Model {
     ...
     * @return boolean
     * $datas[] = array('id'=>1,'name'=>'tom','age'=>8);
     * $datas[] = array('id'=>2,'name'=>'cat','age'=>array('exp','age+1'));
     */
            ...
            foreach ($data as $key=>$value) {
                if($pk===$key){
                    ...
                    if(is_array($value) && $value[0]==='exp'){
                        $lists[$key].= sprintf("WHEN %u THEN %s ",$data[$pk],$value[1]);
                    }else{
     ...
     * @access public
     * @param string|array $field  字段名
     * @param integer $change  增减值
     * @return boolean
     */
    public function setChange($field,$change=''){
        if(!is_array($field)){
            $field = array($field=>$change);
        }
